...
void BmFN(std::string benchmark, int N, int vec_length, int kernel, bool sorted, bool func, bool record, std::string arch, int rowmerge){
    ...
    // if (sparse == 1){
    {
        ...
        MakeDenseMatrix<InType>(1, nonzeros, values, generator);//make a matrix: 1 x (nnz*veclength)
        ...
        if (kernel == 0){
            ...
            for(int i = 0; i < repeat; i++){
                ...
                spmm::TwmmaSpmm(m_vec, vec_length, N, K, d_row_indices, d_row_offsets, d_col_indices, d_value, d_rhs_matrix, d_output_value);
            ...
            Gperf1 = ((double)nnz) * N * 2 / 1000/ 1000 / total_msec * repeat;
            Gperf2 = ((double)nonzeros) * N * 2 / 1000/ 1000 / total_msec * repeat;
            if(rowmerge == 0) Gperf = Gperf2;
            else Gperf = Gperf1;
            ...
            //-------------edit here to change the output file-----------------------------
                ...
                std::string line;
                if(rowmerge == 1){
                    if(vec_length == 8) line = "_vectorsparse_8_v0.csv";
                    else if(vec_length == 16) line = "_vectorsparse_16_v0.csv";
                }
                else if(rowmerge == 2){
                    if(vec_length == 8) line = "_vectorsparse_8_v1.csv";
                    else if(vec_length == 16) line = "_vectorsparse_16_v1.csv";
                }
                else line = "_vectorsparse_structured.csv";
...
void usage(void){
    printf("Input Help!\n");
    printf("Input Format is  ./tspmm_test [paths.txt] [dimN] [vec_length] [kernel] [sort] [func] [mixed] [reocrd] [architecture]\n");
    printf("dimN: N in M-K-N\n");
    printf("veclength: {2, 4, 8, 16}\n");
    printf("kernel: {0: vectorSparse}\n");
    ...
    printf("record: {1: save as csv file; 0: not save}\n");
    printf("GPU(Tensor Core) Architecture: {volta, turing, ampere, ada/hopper}");
    printf("\n");
    printf("\n");
    printf("\n");
    exit(1);
...
int main(int argc, char **argv){
    ...
    const int vec_length = std::atoi(argv[3]);//1 2 4 8
    const int kernel = std::atoi(argv[4]);//0:wmma 1:cuda 2:sputnik 3:cusparse
    ...
    const int rowmerge = std::atoi(argv[9]);//
...

    {
        ...
        while (std::getline(infile, line)){
            ...
            if (mixed) BmFN<half, half, int, float, int>(benchmark, dimN, vec_length, kernel, sorted, func, record, arch, rowmerge);
            else BmFN<half, float, int, float, int>(benchmark, dimN, vec_length, kernel, sorted, func, record, arch, rowmerge); 
            file_count++;
        }
        infile.close();
        printf("num:%d\n", file_count);
